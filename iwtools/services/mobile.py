...
class Mobile:
    ...
    def __init__(self, target, api_key=None, recheck=False, quiet=False):
        ...
        self.session.headers.update(
            {"User-Agent": self.USER_AGENT, "Connection": "close"}
        )
    ...
    def get_cache_id_or_start_test(self):
        ...
        files = None
        data = {"api_key": self.api_key, "hide_in_statistics": 1}
    ...
    def get_result_by_job_id(self, job_id):
        ...
        data = {"job_id": job_id, "verbosity": 1}
    ...
    def wait_test_results(self, test_id):
        ...
        while "status" in response and response["status"] in [
            "in_progress",
            "unprocessed",
            "doesnt_exist",
        ]:
    ...
    async def watch(self, test_id):
        ...
        if self.quiet or logging.root.level == logging.DEBUG:
            ...
            loader = asyncio.ensure_future(
                self.test_loader(
                    "Mobile App Security Test in progress", "Test completed"
                )
            )
    ...
    def start(self):
        ...
        logging.info(
            colored("\nImmuni", "red", attrs=["bold"])
            + colored("Web®", "blue", attrs=["bold"])
        ...
        for task in response:
            ...
            if "status" in task and task["status"] == "error":
                error = task["message"]
                break

        if error:
            raise Exception(self.normalize_text(error))
    ...
    def print_report(self, test_results=None):
        ...
        owasp_color = self.normalize_color(
            test_results["scores"]["owasp_top_10"]["class"]
        )
        behaviour_color = self.normalize_color(
            test_results["scores"]["behaviour"]["class"]
        )
        ...
        banner = self.generate_banner(
            owasp_color, behaviour_color, sca_color, apis_color
        )

        test_time = strftime(
            "%B %d, %Y %H:%M:%S", localtime(test_results["data"]["app_info"]["ts_stop"])
        )
        ...
        logging.info(
            colored("App Name: ", attrs=["bold"])
            + test_results["data"]["app_info"]["app_name"]
        )
        logging.info(
            colored("App ID: ", attrs=["bold"])
            + test_results["data"]["app_info"]["app_id"]
        )
        logging.info(
            colored("Version: ", attrs=["bold"])
            + test_results["data"]["app_info"]["app_version"]
        )
        logging.info(
            colored("Developer: ", attrs=["bold"])
            + test_results["data"]["app_info"]["app_developer"]
        )
        logging.info(
            colored("OS: ", attrs=["bold"])
            + test_results["data"]["app_info"]["device_type"].title()
        )
        logging.info(
            colored("Completed: ", attrs=["bold"]) + colored(test_time, test_time_color)
        )
        logging.info(banner)
        logging.info(
            colored("OWASP Mobile Top 10 Security Test: ", attrs=["bold"])
            + colored(
                test_results["scores"]["owasp_top_10"]["description"].title(),
                owasp_color,
            )
        )
        logging.info(
            colored("Mobile App Privacy and Behavior: ", attrs=["bold"])
            + colored(
                test_results["scores"]["behaviour"]["description"].title(),
                behaviour_color,
            )
        )
        logging.info(
            colored("Software Composition Analysis Test: ", attrs=["bold"])
            + colored(test_results["scores"]["sca"]["description"].title(), sca_color)
        )
        logging.info(
            colored("Mobile App External Communications: ", attrs=["bold"])
            + colored(test_results["scores"]["apis"]["description"].title(), apis_color)
        )
